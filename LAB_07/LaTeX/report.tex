...
\newcommand{\IDone}{\texttt{[ID1]}}   % <-- Saleh Khalil, ID
\newcommand{\IDtwo}{\texttt{[ID2]}}   % <-- Mahmood Isteteh, ID
\newcommand{\youtubelink}{https://youtu.be/W0P9rKjtpL8}  % <-- paste your YouTube demo URL here
...
\begin{itemize}
  \item \textbf{\texttt{Ps2\_Interface.v}} -- Receives PS/2 keyboard frames
    ...
    \begin{table}[H]
      ...
      \begin{tabular}{lllp{0.42\textwidth}}
        ...
        \textbf{Port} & \textbf{Dir} & \textbf{Width} & \textbf{Description} \\
        \midrule
        \texttt{PS2Clk}  & in  & 1 & Keyboard-generated clock; data sampled
          on its falling edge. \\
  ...
  \item \textbf{\texttt{Debouncer.v}} -- Converts the bouncy centre
    ...
    \begin{table}[H]
      \centering\small
      \caption{\texttt{Debouncer} interface (parameter \texttt{COUNTER\_BITS}).}
      \begin{tabular}{lllp{0.42\textwidth}}
        \toprule
        \textbf{Port} & \textbf{Dir} & \textbf{Width} & \textbf{Description} \\
        \midrule
        \texttt{clk} & in & 1 & System clock. \\
        \texttt{input\_unstable} & in & 1 & Raw (bouncy) push-button input. \\
        \texttt{output\_stable} & out & 1 & One-clock pulse once the
          hysteresis counter saturates high. \\
        \bottomrule
      \end{tabular}
    \end{table}
  ...
  \item \textbf{\texttt{VGA\_Interface.v}} -- Generates VGA signals for
    ...
    \begin{table}[H]
      \centering\small
      \caption{\texttt{VGA\_Interface} interface.}
      \begin{tabular}{lllp{0.42\textwidth}}
        \toprule
        \textbf{Port} & \textbf{Dir} & \textbf{Width} & \textbf{Description} \\
        \midrule
        \texttt{clk} & in & 1 & 100\,MHz clock (internal \texttt{pix\_en}
          enable every second cycle). \\
        \texttt{rstn} & in & 1 & Active-low reset. \\
        \texttt{pixel\_color} & in & 12 & RGB color of the current pixel
          from \texttt{Pixel\_Painter}. \\
        \texttt{vgaRed/Green/Blue} & out & $3\times4$ & VGA color pins,
          blanked outside the visible region. \\
        \texttt{Hsync}, \texttt{Vsync} & out & 1 & VGA synchronisation pulses. \\
        \texttt{XCoord}, \texttt{YCoord} & out & 11 & Current scan position
          towards \texttt{Pixel\_Painter}. \\
        \bottomrule
      \end{tabular}
    \end{table}
  ...
  \item \textbf{\texttt{Seg\_7\_Display.v}} -- Shows the 2-digit score supplied by the \texttt{Snake.v} module on the 7-segment display.
    ...
    \begin{table}[H]
      \centering\small
      \caption{\texttt{Seg\_7\_Display} interface.}
      \begin{tabular}{lllp{0.42\textwidth}}
        \toprule
        \textbf{Port} & \textbf{Dir} & \textbf{Width} & \textbf{Description} \\
        \midrule
        \texttt{x} & in & 16 & Value to display (the score), one hex
          nibble per digit. \\
        \texttt{clk} & in & 1 & 100\,MHz clock for the digit multiplexing. \\
        \texttt{clr} & in & 1 & Clears the digit-multiplex divider. \\
        \texttt{a\_to\_g} & out & 7 & Segment toggles (active low). \\
        \texttt{an} & out & 4 & Digit anodes (active low, one at a time). \\
        \texttt{dp} & out & 1 & Decimal-point toggle. \\
        \bottomrule
      \end{tabular}
    \end{table}
\end{itemize}
...
This section covers the new modules that implement the game itself:
direction control, the snake core with its food generator, the movement
tick and the rendering pipeline.
% \placeholderbox{5cm}{Game-logic block highlighted: the snake core, its food
% generator and the food-coverage testbench.}

\subsection{\texttt{Navigation\_System.v}}
\textbf{Purpose.} Maps keyboard scancodes to the 2-bit snake direction
\texttt{dir} and blocks immediate $180^{\circ}$ reversals.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{hierarchy_Navigation_System.png}
  \caption{\texttt{Navigation\_System.v} highlighted in the design hierarchy.}
  \label{fig:hier-nav}
\end{figure}

\begin{table}[H]
  \centering\small
  \caption{\texttt{Navigation\_System} interface.}
  \begin{tabular}{lllp{0.45\textwidth}}
    \toprule
    \textbf{Port} & \textbf{Dir} & \textbf{Width} & \textbf{Description} \\
    \midrule
    \texttt{clk}, \texttt{reset} & in & 1 & Clock and synchronous reset. \\
    \texttt{scancode} & in & 8 & Make code from \texttt{Ps2\_Interface}. \\
    \texttt{keyPressed} & in & 1 & One-clock strobe qualifying \texttt{scancode}. \\
    \texttt{dir} & out & 2 & Current direction towards \texttt{Snake}
      (UP/DOWN/LEFT/RIGHT). \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{Internal design.} A clocked \texttt{case} on \texttt{scancode},
qualified by \texttt{keyPressed}. Each assignment is guarded by
``\texttt{if (dir != opposite)}'', which implements the
no-immediate-reversal requirement directly in hardware. On reset the
direction defaults to RIGHT. Any other key leaves \texttt{dir} unchanged
(but still wakes the game from the welcome screen through
\texttt{keyPressed}).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{hierarchy_snake.png}
  \caption{\texttt{snake.v} highlighted in the design hierarchy.}
  \label{fig:hier-nav}
\end{figure}


\textbf{Purpose.} Owns the snake body, movement, growth, collision
detection, score and food position. It is the heart of the game.
...
\begin{table}[H]
  ...
  \begin{tabular}{lllp{0.42\textwidth}}
    ...
    \texttt{crash} & out & 1 & Game-over flag. \\
...
\textbf{Verification.} We tested it as part of \texttt{snake\_game\_tb}
(movement, steering, growth, crash, restart).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{hierarchy_farmer_farmer_tb.png}
  \caption{\texttt{farmer.v} and \texttt{farmer\_tb.v} highlighted in the
  design hierarchy.}
  \label{fig:farmer}
\end{figure}

\textbf{Purpose.} This module is a pseudo-random food-coordinate
generator for the food position on the board (\texttt{food\_x},
\texttt{food\_y}).

\begin{table}[H]
  \centering\small
  \caption{\texttt{farmer} interface (parameters \texttt{GRID\_X}, \texttt{GRID\_Y}).}
  \begin{tabular}{lllp{0.45\textwidth}}
    \toprule
    \textbf{Port} & \textbf{Dir} & \textbf{Width} & \textbf{Description} \\
    \midrule
    \texttt{clk} & in & 1 & 100\,MHz clock; the LFSR shifts every cycle. \\
    \texttt{keyPressed} & in & 1 & Keyboard strobe mixed into the LFSR
      feedback as an entropy source. \\
    \texttt{food\_x} & out & 7 & Proposed food column,
      $\texttt{lfsr}[6{:}0] \bmod \texttt{GRID\_X}$. \\
    \texttt{food\_y} & out & 7 & Proposed food row,
      $\texttt{lfsr}[13{:}7] \bmod \texttt{GRID\_Y}$. \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Internal design.} It holds a 16-bit shift register that shifts
left every clock (100\,MHz \texttt{clk}), where the new LSB is the
\texttt{Xor} of \texttt{lfsr[15]}, \texttt{lfsr[13]}, \texttt{lfsr[12]},
\texttt{lfsr[10]} and \texttt{keyPressed}. The output looks random but
is in reality fully deterministic, so \texttt{keyPressed} is mixed in on
every clock as an entropy source to make the output less predictable.
The food coordinates are computed as follows:
$\texttt{food\_x} = \texttt{lfsr}[6{:}0] \bmod \texttt{GRID\_X}$ and
$\texttt{food\_y} = \texttt{lfsr}[13{:}7] \bmod \texttt{GRID\_Y}$.

\textbf{Verification.} The testbench clocks the shift register
$15{,}000$ times ($\approx 2$ proposals per grid cell), toggles
\texttt{keyPressed} randomly ($\approx 1\%$ of clocks) and records every
proposed coordinate.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{farmer_wavefrom.png}
  \caption{Simulation waveform of \texttt{farmer\_tb}: \texttt{food\_x}
  and \texttt{food\_y} change every clock, showing that the generator
  keeps proposing fresh coordinates.}
  \label{fig:farmer-waveform}
\end{figure}

\textbf{Coverage.} Figure~\ref{fig:coverage} shows the resulting
coverage of the $100\times75$ grid: green cells were proposed at least
once, dark cells never. We achieved $\approx 81\%$ coverage, so the
farmer is good enough to generate food coordinates across the board.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{hierarchy_game_tick_tick_tb.png}
  \caption{Timing block highlighted.}
\end{figure}

\begin{table}[H]
  \centering\small
  \caption{\texttt{Game\_Tick} interface (parameter \texttt{TICK\_MAX}).}
  \begin{tabular}{lllp{0.45\textwidth}}
    \toprule
    \textbf{Port} & \textbf{Dir} & \textbf{Width} & \textbf{Description} \\
    \midrule
    \texttt{clk}, \texttt{reset} & in & 1 & Clock and synchronous reset. \\
    \texttt{score} & in & 16 & Current score from \texttt{Snake};
      shortens the tick period as it grows. \\
    \texttt{tick} & out & 1 & One-clock movement strobe towards
      \texttt{Snake}. \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Internal design.} A counter \texttt{(Lim\_inc from lab2)} compares against a programmable
...
\textbf{Verification -- \texttt{tick\_tb.v}.} The  testbench sweeps
the score \texttt{from 0 to MAX\_LEN(=64)} and
exposes the tick pulse and at each $clk$ calculates the frequency.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{tick_wavefrom.png}
  \caption{tick\_tb waveform}
\end{figure}
...
\textbf{Purpose} Bridges \texttt{VGA\_Interface.v} and
\texttt{grid\_mapper.v}, translating between the pixel domain (VGA scan
coordinates) and the cell domain (game grid), and composes the final
pixel color.

\begin{table}[H]
  \centering\small
  \caption{\texttt{Pixel\_Painter} interface (parameters \texttt{GRID\_X}, \texttt{GRID\_Y}).}
  \begin{tabular}{lllp{0.42\textwidth}}
    \toprule
    \textbf{Port} & \textbf{Dir} & \textbf{Width} & \textbf{Description} \\
    \midrule
    \texttt{clk}, \texttt{reset} & in & 1 & Clock and synchronous reset. \\
    \texttt{tick} & in & 1 & Movement strobe from \texttt{Game\_Tick}. \\
    \texttt{keyPressed} & in & 1 & Key strobe, forwarded to the screen FSM. \\
    \texttt{crash} & in & 1 & Game-over flag from \texttt{Snake}. \\
    \texttt{on\_snake}, \texttt{is\_head}, \texttt{is\_food} & in & 1 &
      Query answers from \texttt{Snake} for cell $(x,y)$. \\
    \texttt{XCoord}, \texttt{YCoord} & in & 11 & VGA scan coordinates
      from \texttt{VGA\_Interface}. \\
    \texttt{x}, \texttt{y} & out & 7/7 & Queried grid cell
      (scan position $\div\,8$). \\
    \texttt{pixel\_color} & out & 12 & Final RGB color towards
      \texttt{VGA\_Interface}. \\
    \texttt{start\_game} & out & 1 & High while the PLAY screen is shown;
      releases the snake core from reset. \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Internal design.} The same VGA scan position is translated two
ways in parallel, one per screen. For the PLAY screen, it is divided by
8 (\texttt{XCoord >> 3}, \texttt{YCoord >> 3}) to obtain the queried
grid cell $(x,y)$, which \texttt{GridMapper} uses for the snake/food/
checkerboard logic. For the IDLE and GAME\_OVER screens, it is instead
divided by 4 (\texttt{XCoord >> 2}, \texttt{YCoord >> 2}) to obtain the
$200\times150$ bitmap coordinates $(\texttt{img\_x}, \texttt{img\_y})$
used to index the welcome and skull splash screens. The cell color
returned by \texttt{GridMapper} is overlaid with black 1-pixel grid
lines (every 8th row/column) while the game is in PLAY;
\texttt{start\_game} (\ ``the game is on the PLAY screen'') is
exported to the top level, where it releases the snake core from reset.
...
\begin{table}[H]
  \centering\small
  \caption{\texttt{GridMapper} interface (parameters \texttt{GRID\_X}, \texttt{GRID\_Y}).}
  \begin{tabular}{lllp{0.42\textwidth}}
    \toprule
    \textbf{Port} & \textbf{Dir} & \textbf{Width} & \textbf{Description} \\
    \midrule
    \texttt{clk}, \texttt{reset} & in & 1 & Clock and synchronous reset. \\
    \texttt{keyPressed} & in & 1 & Advances the screen FSM
      (IDLE/GAME\_OVER $\rightarrow$ PLAY). \\
    \texttt{crash} & in & 1 & Moves the FSM from PLAY to GAME\_OVER. \\
    \texttt{on\_snake}, \texttt{is\_head}, \texttt{is\_food} & in & 1 &
      Cell content flags from \texttt{Snake}. \\
    \texttt{x}, \texttt{y} & in & 7/7 & Queried grid cell from
      \texttt{Pixel\_Painter}. \\
    \texttt{img\_x}, \texttt{img\_y} & in & 8/8 & Bitmap coordinates for
      the welcome / game-over screens. \\
    \texttt{grid\_enable} & out & 1 & High in PLAY; enables the grid-line
      overlay in \texttt{Pixel\_Painter}. \\
    \texttt{block\_color} & out & 12 & Color of the queried cell for the
      current screen. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{llll}
    ...
    Food       & \texttt{0xF11} & GAME\_OVER background & \texttt{0x000} \\
...
board (texttt{xc7a35tlcpg236-2L}) Synthesis and implementation
...
\begin{figure}[H]\centering
  \includegraphics[width=0.8\textwidth]{netlist_schematic_flip.pdf}
...
\begin{figure}[H]\centering
  \begin{minipage}{0.485\textwidth}\centering
    \includegraphics[width=0.9\linewidth]{photo_welcome.png}\\[3pt]
    \small (a) Welcome / start screen
  \end{minipage}\hfill
  \begin{minipage}{0.485\textwidth}\centering
    \includegraphics[width=0.9\linewidth]{photo_play1.png}\\[3pt]
    \small (b) Playing --- snake and red food
  \end{minipage}

  \vspace{10pt}
  \begin{minipage}{0.485\textwidth}\centering
    \includegraphics[width=0.9\linewidth]{photo_play2.png}\\[3pt]
    \small (c) Later in the round --- longer snake
  \end{minipage}\hfill
  \begin{minipage}{0.485\textwidth}\centering
    \includegraphics[width=0.9\linewidth]{photo_gameover.png}\\[3pt]
