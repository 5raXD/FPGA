...
game on the Digilent Basys3 board (Xilinx Artix-7, XC7A35T). The game is
...
steers the snake with a PS/2 keyboard. Unlike previous experiments, no
skeleton code was supplied for the game itself: the game modules were
designed from scratch, while proven building blocks from previous labs
(PS/2 interface, VGA interface, 7-segment driver, debouncer, full adder)
were reused and adapted.
...
Figure~\ref{fig:hier-all} shows the complete design: every Verilog file,
the hierarchy (who instantiates whom), the dataflow between the blocks,
and the three testbenches. Colours indicate the role of each block.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{hierarchy_all.png}
  \caption{Full design map: file hierarchy, dataflow and testbenches.
  Blue = interface/IO, green = core logic, yellow = generators,
  purple = rendering, dashed = testbenches.}
  \label{fig:hier-all}
\end{figure}
...
\begin{enumerate}
  ...
  \item \texttt{Pixel\_Painter} + \texttt{GridMapper} convert the VGA
        ...
        colour for the current screen (welcome / play / game over).
...
\begin{table}[H]
  \centering\small
  \caption{All Verilog files, their modules and origin.}
  \label{tab:files}
  \begin{tabular}{llll}
    \toprule
    ...
    \texttt{Debouncer.v}          & \texttt{Debouncer}          & button conditioning  & reuse (Lab 1) \\
    \texttt{game\_tick.v}         & \texttt{Game\_Tick}         & movement strobe      & new \\
    ...
    \texttt{grid\_mapper.v}       & \texttt{GridMapper}         & rendering FSM        & new \\
    \texttt{FA.v}                 & \texttt{FA}                 & full adder           & reuse (Lab 1) \\
    \texttt{VGA\_Interface.v}     & \texttt{VGA\_Interface}     & VGA sync/scan        & reuse (Lab 6) \\
    ...
    \texttt{farmer\_tb.v}         & \texttt{farmer\_tb}         & unit testbench        & new \\
    \texttt{tick\_tb.v}           & \texttt{tick\_tb}           & unit testbench        & new \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{Purpose.} Wires all blocks together and maps the design to the
Basys3 pins (constraints in \texttt{task3.xdc}). It contains no game
logic of its own -- only instantiations and the shared nets
(\texttt{dir}, \texttt{tick}, \texttt{score}, cell-query signals,
\texttt{pixel\_color}).

\begin{table}[H]
  \centering\small
  \caption{\texttt{snake\_game} interface.}
  \begin{tabular}{lllp{0.45\textwidth}}
    ...
    \texttt{vgaRed/Green/Blue} & out & $3\times4$ & VGA colour channels. \\
...
(IDLE $\rightarrow$ PLAY $\rightarrow$ GAME\_OVER). Annotate with arrows:
\texttt{keyPressed} pulses, \texttt{dir} changes, \texttt{tick} strobes,
head coordinates advancing, \texttt{crash} rising at the wall, and the
\texttt{state} transition to GAME\_OVER.}
...
% =====================================================================
\section{Game Logic Block}
% =====================================================================
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{hierarchy_farmer_snake_farmer_tb.png}
  \caption{Game-logic block highlighted: the snake core, its food
...
\subsection{\texttt{farmer.v} -- food generator}
\textbf{Purpose.} Proposes pseudo-random food coordinates.

\textbf{Internal design.} A 16-bit Fibonacci LFSR (taps 15, 13, 12, 10,
seed \texttt{0xABCD}) shifts every clock; the keyboard strobe
\texttt{keyPressed} is XOR-ed into the feedback as a source of true
user-driven entropy. The outputs are
$\texttt{food\_x} = \texttt{lfsr}[6{:}0] \bmod \texttt{GRID\_X}$ and
$\texttt{food\_y} = \texttt{lfsr}[13{:}7] \bmod \texttt{GRID\_Y}$.
Since the LFSR runs at 100\,MHz, the value latched by \texttt{snake} at
eat time is effectively random with respect to the game timescale.

\textbf{Verification -- \texttt{farmer\_tb.v} and coverage.} The unit
testbench clocks the LFSR $15{,}000$ times ($\approx 2$ proposals per
grid cell), toggles \texttt{keyPressed} randomly ($\approx 1\%$ of
clocks) and records every proposed coordinate. Figure~\ref{fig:coverage}
shows the resulting coverage of the $100\times75$ grid: green cells were
proposed at least once, dark cells never.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{hierarchy_Pixel_Painter_grid_mapper_FA.png}
  \caption{Rendering block highlighted.}
\end{figure}

\subsection{\texttt{Pixel\_Painter.v}}
...
screens. The cell colour returned by \texttt{GridMapper} is overlaid
...
\subsection{\texttt{grid\_mapper.v}}
\textbf{Purpose.} The screen FSM and colour oracle: decides \emph{what
...
\begin{table}[H]
  \centering\small
  \caption{Colour palette (12-bit RGB) in \texttt{grid\_mapper.v}.}
  \begin{tabular}{llll}
    \toprule
    \textbf{Element} & \textbf{Colour} & \textbf{Element} & \textbf{Colour} \\
    \midrule
...
the RGB pins with the colour supplied by the renderer, and exports the
scan coordinates.
...
\placeholderbox{4.6cm}{Photo of the monitor during PLAY: snake, red
food, checkerboard background and grid lines visible.}

\placeholderbox{4.6cm}{Photo of the game-over (skull) screen on the
monitor, and of the welcome splash.}

\placeholderbox{4.6cm}{Photo of the Basys3 board during a game: score on
the 7-segment display, VGA and PS/2 cables connected.}

\subsection{Working game}
% -----------------------------------------------------------------
% When the photo/screenshot is ready, copy it next to report.tex
% (e.g. working_game.png) and replace the placeholder box below with:
%     \begin{figure}[H]\centering
%       \includegraphics[width=0.85\textwidth]{working_game.png}
%       \caption{The Snake game running on the monitor.}
%     \end{figure}
% -----------------------------------------------------------------
\placeholderbox{6.5cm}{Image of the working game --- photo/screenshot of
the monitor running the game. Drop the image (e.g.\ \texttt{working\_game.png})
into this folder and replace this box with an \texttt{includegraphics} of it.}
